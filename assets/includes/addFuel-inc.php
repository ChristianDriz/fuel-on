...
if(isset($_POST['save'])){
    ...
    $data = 'fuelname=' .$fuelname. '&price=' .$price. '&category=' .$category. '&fuel_status=' .$fuel_status;
}

	if(empty($fuelname) || empty($price) || empty($category) || empty($fuel_status)){
		$dbh->info("../../store-myfuels.php?$data", "All fields should be filled out!");
	}
	else{
		date_default_timezone_set('Asia/Manila');
		$date = date("Y-m-d H:i:s");

		$dbh->insertFuel($fuelname, $category, $price, $fuel_status, $date, $userID);
		$dbh->success("../../store-myfuels.php", "Fuel added successfully!");  	
	}
	...
	// 				$dbh->insertFuel($fuelname, $category, $image, $price, $fuel_status, $date, $userID);
